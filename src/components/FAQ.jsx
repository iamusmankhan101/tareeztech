        ...
        <div className="faq-list">
          {faqs.map((faq, index) => {
            ...
            return (
              ...
              >
                <button 
                  className="faq-question w-full text-left flex justify-between items-center py-6 px-4 md:px-6 bg-transparent border-none cursor-pointer focus:outline-none"
                  onClick={() => toggleFaq(index)}
                >
                  <span className="text-lg md:text-xl font-medium text-white">{faq.question}</span>
                  <span className={`faq-icon ml-4 flex-shrink-0 transition-transform duration-300 ${isActive ? 'rotate-180' : ''}`}>
                ...
                <AnimatePresence>
                  {isActive && (
                    ...
                    >
                      <div className="faq-answer pb-6 px-4 md:px-6">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
